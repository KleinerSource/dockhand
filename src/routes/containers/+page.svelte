<svelte:head>
	<title>{$t('containers.page.pageTitle')}</title>
</svelte:head>
...
<script lang="ts">
	...
	const statusTypes = $derived([
		{ value: 'running', label: $t('containers.status.running'), icon: Play, color: 'text-emerald-500' },
		{ value: 'paused', label: $t('containers.status.paused'), icon: Pause, color: 'text-amber-500' },
		{ value: 'restarting', label: $t('containers.status.restarting'), icon: RotateCw, color: 'text-red-500' },
		{ value: 'exited', label: $t('containers.status.exited'), icon: Square, color: 'text-rose-500' },
		{ value: 'created', label: $t('containers.status.created'), icon: Plus, color: 'text-sky-500' },
		{ value: 'dead', label: $t('containers.status.dead'), icon: Skull, color: 'text-gray-500' }
	]);
	...
	function getStatusLabelKey(state: string): string {
		...
		return statusTypes.some(s => s.value === normalized)
	...
	function bulkStart() {
		startBatchOperation(
			translate('containers.batch.titles.starting', { count: selectedStopped.length }),
			'start',
			selectedStopped
		);
	}
	...
	function bulkStop() {
		startBatchOperation(
			translate('containers.batch.titles.stopping', { count: selectedRunning.length }),
			'stop',
			selectedRunning
		);
	}
	...
	function bulkRestart() {
		startBatchOperation(
			translate('containers.batch.titles.restarting', { count: selectedNonSystem.length }),
			'restart',
			selectedNonSystem
		);
	}
	...
	function bulkPause() {
		startBatchOperation(
			translate('containers.batch.titles.pausing', { count: selectedRunning.length }),
			'pause',
			selectedRunning
		);
	}
	...
	function bulkUnpause() {
		startBatchOperation(
			translate('containers.batch.titles.unpausing', { count: selectedPaused.length }),
			'unpause',
			selectedPaused
		);
	}
	...
	function bulkRemove() {
		startBatchOperation(
			translate('containers.batch.titles.removing', { count: selectedNonSystem.length }),
			'remove',
			selectedNonSystem,
			{ force: true }
		);
	...
	function showFailedChecksToast(failed: typeof failedUpdateChecks, prefix: string) {
		...
		toast.warning(translate('containers.toasts.failedUpdateChecksSummary', { prefix, count: failed.length }), {
			...
			duration: Infinity,
			action: {
				label: translate('common.actions.ok'),
				onClick: () => {}
			}
		});
	}
		...
		try {
			...
			if (containersWithUpdates.length === 0) {
				...
				if (failed.length > 0) {
					...
					showFailedChecksToast(failedUpdateChecks, translate('containers.toasts.allUpToDate'));
					pendingTimeouts.push(setTimeout(() => { updateCheckStatus = 'idle'; }, 3000));
				} else {
					updateCheckStatus = 'none';
					toast.success(translate('containers.toasts.allUpToDate'));
					pendingTimeouts.push(setTimeout(() => { updateCheckStatus = 'idle'; }, 3000));
				}
				return;
			}
			...
			if (failed.length > 0) {
				showFailedChecksToast(failedUpdateChecks, translate('containers.toasts.updatesAvailable', { count: containersWithUpdates.length }));
			} else {
				toast.info(translate('containers.toasts.updatesAvailable', { count: containersWithUpdates.length }));
			}
		} catch (error) {
			updateCheckStatus = 'error';
			pendingTimeouts.push(setTimeout(() => { updateCheckStatus = 'idle'; }, 3000));
	...
	async function dismissPendingUpdates() {
		...
		try {
			...
			toast.error(translate('containers.toasts.clearUpdateIndicatorsFailed'));
		}
	}
	...
	function handleBatchUpdateComplete(results: { success: string[]; failed: string[]; blocked: string[] }) {
		if (results.success.length > 0) {
			toast.success(translate('containers.toasts.updatedContainers', { count: results.success.length }));
		}
		if (results.failed.length > 0) {
			toast.error(translate('containers.toasts.updateContainersFailed', { count: results.failed.length }));
		}
		if (results.blocked.length > 0) {
			toast.warning(translate('containers.toasts.updatesBlockedByPolicy', { count: results.blocked.length }));
		}
		selectedContainers = new Set();
	...
	async function startContainer(id: string) {
		...
		try {
			...
			if (!response.ok) {
				...
				operationError = { id, message: data.error || translate('containers.errors.startContainer') };
				toast.error(translate('containers.toasts.startFailed', { name }));
				clearErrorAfterDelay(id);
				return;
			}
			toast.success(translate('containers.toasts.started', { name }));
			await containerStore.refreshContainers(envId);
		} catch (error) {
			console.error('Failed to start container:', error);
			operationError = { id, message: translate('containers.errors.startContainer') };
			toast.error(translate('containers.toasts.startFailed', { name }));
			clearErrorAfterDelay(id);
		}
	}
	...
	async function stopContainer(id: string) {
		...
		try {
			...
			if (!response.ok) {
				const data = await response.json();
				operationError = { id, message: data.error || translate('containers.errors.stopContainer') };
				toast.error(translate('containers.toasts.stopFailed', { name }));
				clearErrorAfterDelay(id);
				return;
			}
			toast.success(translate('containers.toasts.stopped', { name }));
			await containerStore.refreshContainers(envId);
		} catch (error) {
			console.error('Failed to stop container:', error);
			operationError = { id, message: translate('containers.errors.stopContainer') };
			toast.error(translate('containers.toasts.stopFailed', { name }));
			clearErrorAfterDelay(id);
		} finally {
			stoppingId = null;
	...
	async function pauseContainer(id: string) {
		...
		try {
			...
			if (!response.ok) {
				const data = await response.json();
				operationError = { id, message: data.error || translate('containers.errors.pauseContainer') };
				toast.error(translate('containers.toasts.pauseFailed', { name }));
				clearErrorAfterDelay(id);
				return;
			}
			toast.success(translate('containers.toasts.paused', { name }));
			await containerStore.refreshContainers(envId);
		} catch (error) {
			console.error('Failed to pause container:', error);
			operationError = { id, message: translate('containers.errors.pauseContainer') };
			toast.error(translate('containers.toasts.pauseFailed', { name }));
			clearErrorAfterDelay(id);
		}
	}
	...
	async function unpauseContainer(id: string) {
		...
		try {
			...
			if (!response.ok) {
				const data = await response.json();
				operationError = { id, message: data.error || translate('containers.errors.unpauseContainer') };
				toast.error(translate('containers.toasts.unpauseFailed', { name }));
				clearErrorAfterDelay(id);
				return;
			}
			toast.success(translate('containers.toasts.resumed', { name }));
			await containerStore.refreshContainers(envId);
		} catch (error) {
			console.error('Failed to unpause container:', error);
			operationError = { id, message: translate('containers.errors.unpauseContainer') };
			toast.error(translate('containers.toasts.unpauseFailed', { name }));
			clearErrorAfterDelay(id);
		}
	}
	...
	async function restartContainer(id: string) {
		...
		try {
			...
			if (!response.ok) {
				const data = await response.json();
				operationError = { id, message: data.error || translate('containers.errors.restartContainer') };
				toast.error(translate('containers.toasts.restartFailed', { name }));
				clearErrorAfterDelay(id);
				return;
			}
			toast.success(translate('containers.toasts.restarted', { name }));
			await containerStore.refreshContainers(envId);
		} catch (error) {
			console.error('Failed to restart container:', error);
			operationError = { id, message: translate('containers.errors.restartContainer') };
			toast.error(translate('containers.toasts.restartFailed', { name }));
			clearErrorAfterDelay(id);
		} finally {
			restartingId = null;
	...
	async function removeContainer(id: string) {
		...
		try {
			...
			if (!response.ok) {
				const data = await response.json();
				operationError = { id, message: data.error || translate('containers.errors.removeContainer') };
				toast.error(translate('containers.toasts.removeFailed', { name }));
				clearErrorAfterDelay(id);
				return;
			}
			toast.success(translate('containers.toasts.removed', { name }));
			await containerStore.refreshContainers(envId);
		} catch (error) {
			console.error('Failed to remove container:', error);
			operationError = { id, message: translate('containers.errors.removeContainer') };
			toast.error(translate('containers.toasts.removeFailed', { name }));
			clearErrorAfterDelay(id);
		}
	}
	...
	async function copyCommand(text: string) {
		...
		if (ok) {
			copiedCommand = text;
			toast.success(translate('containers.toasts.copiedToClipboard'));
...
<div class="flex-1 min-h-0 flex flex-col gap-3 overflow-hidden">
	<div class="shrink-0 flex flex-wrap justify-between items-center gap-3 min-h-8">
		<PageHeader icon={Box} title={$t('containers.page.title')} count={containers.length} />
		<div class="flex flex-wrap items-center gap-2">
			<div class="relative">
				<Search class="absolute left-2 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground" />
				<Input
					type="text"
					placeholder={$t('containers.page.searchPlaceholder')}
			...
			<MultiSelectFilter
				...
				options={statusTypes}
				placeholder={$t('containers.filters.allStatuses')}
				pluralLabel={$t('containers.filters.statuses')}
				width="w-44"
				defaultIcon={Box}
			/>
			<div class="flex gap-2">
				...
				<Button
					...
					title={$t('containers.actions.checkUpdates')}
					class="relative overflow-hidden"
				>
					{#if updateCheckStatus === 'checking'}
						<CircleArrowUp class="w-3.5 h-3.5 animate-spin" />
						{#if updateCheckProgress.total > 0}
							<span class="tabular-nums">{$t('containers.actions.checkingProgress', { checked: String(updateCheckProgress.checked).padStart(String(updateCheckProgress.total).length, '\u2007'), total: updateCheckProgress.total })}</span>
							<div
							...
							></div>
						{:else}
							{$t('containers.actions.checkUpdates')}
						{/if}
				...
				<ConfirmPopover
					...
					action={$t('containers.actions.update')}
					itemType={$t('containers.confirm.allUpdatableContainers', { count: updatableContainersCount })}
					confirmText={$t('containers.actions.updateAll')}
					variant="default"
					title={$t('containers.actions.updateAllWithUpdates')}
					position="left"
				...
				>
					{#snippet children({ open })}
						...
						>
							<CircleArrowUp class="w-3.5 h-3.5" />
							{$t('containers.actions.updateAllCount', { count: updatableContainersCount })}
							<button
								type="button"
								...
								title={$t('containers.actions.dismissAllUpdates')}
							>×</button>
						</Button>
					{/snippet}
				</ConfirmPopover>
				...
				<ConfirmPopover
					open={confirmPrune}
					action={$t('containers.actions.prune')}
					itemType={$t('containers.confirm.stoppedContainers')}
					title={$t('containers.actions.pruneContainers')}
					position="left"
				...
				>
					{#snippet children({ open })}
						<Button size="sm" variant="outline" disabled={pruneStatus === 'pruning'}>
							...
							{/if}
							{$t('containers.actions.prune')}
						</Button>
					{/snippet}
				</ConfirmPopover>
				{/if}
				<Button size="sm" variant="outline" onclick={fetchContainers}>{$t('common.actions.refresh')}</Button>
				<Button
					size="sm"
					variant="outline"
					onclick={toggleLayoutMode}
					class="h-8 w-8 p-0"
					title={layoutMode === 'horizontal' ? $t('containers.layout.switchToVertical') : $t('containers.layout.switchToHorizontal')}
				>
	...
	<div class="h-4 shrink-0">
		{#if selectedContainers.size > 0}
			...
			<span>{$t('containers.selection.selected', { count: selectedInFilter.length })}</span>
			<button
				type="button"
			...
			{#if selectedStopped.length > 0 && $canAccess('containers', 'start')}
				<ConfirmPopover
					...
					action={$t('containers.actions.start')}
					itemType={$t('containers.confirm.stoppedContainerCount', { count: selectedStopped.length })}
					title={$t('containers.actions.startCount', { count: selectedStopped.length })}
					variant="secondary"
					unstyled
					onConfirm={bulkStart}
			...
			{#if selectedRunning.length > 0 && $canAccess('containers', 'stop')}
				<ConfirmPopover
					...
					itemType={$t('containers.confirm.runningContainerCount', { count: selectedRunning.length })}
					title={$t('containers.actions.stopCount', { count: selectedRunning.length })}
					unstyled
					onConfirm={bulkStop}
				...
				<ConfirmPopover
					...
					itemType={$t('containers.confirm.runningContainerCount', { count: selectedRunning.length })}
					title={$t('containers.actions.pauseCount', { count: selectedRunning.length })}
					variant="secondary"
					unstyled
					onConfirm={bulkPause}
			...
			{#if selectedPaused.length > 0 && $canAccess('containers', 'start')}
				<ConfirmPopover
					...
					action={$t('containers.actions.unpause')}
					itemType={$t('containers.confirm.pausedContainerCount', { count: selectedPaused.length })}
					title={$t('containers.actions.unpauseCount', { count: selectedPaused.length })}
					variant="secondary"
					unstyled
			...
			<ConfirmPopover
				...
				itemType={$t('containers.confirm.containerCount', { count: selectedNonSystem.length })}
				title={$t('containers.actions.restartCount', { count: selectedNonSystem.length })}
				variant="secondary"
				unstyled
			...
			<ConfirmPopover
				...
				itemType={$t('containers.confirm.containerCount', { count: selectedNonSystem.length })}
				title={$t('containers.actions.removeCount', { count: selectedNonSystem.length })}
				unstyled
			...
			<ConfirmPopover
				...
				action={$t('containers.actions.update')}
				itemType={$t('containers.confirm.selectedContainerCount', { count: selectedWithUpdatesCount })}
				confirmText={$t('containers.actions.update')}
				variant="default"
				title={$t('containers.actions.updateSelectedToLatest')}
			...
			>
				{#snippet children({ open })}
					...
					>
						...
						{$t('containers.actions.updateCount', { count: selectedWithUpdatesCount })}
					</span>
				{/snippet}
			</ConfirmPopover>
			{/if}
	...
	{:else if !loading && containers.length === 0}
		<EmptyState
			icon={Box}
			title={$t('containers.page.emptyTitle')}
			description={$t('containers.page.emptyDescription')}
		/>
	{:else}
				...
				{#snippet cell(column, container, rowState)}
										...
										{#if container.systemContainer === 'dockhand'}
											{#if hasUpdate}
												<div class="space-y-2">
													...
													>
														{$t('containers.system.settingsAbout')}
													</a>
												</div>
											{:else}
												<p class="text-sm whitespace-nowrap">{$t('containers.system.dockhandManagement')}</p>
											{/if}
										{:else}
											{#if hasUpdate}
												<div class="space-y-2">
													...
													<p class="text-muted-foreground text-xs whitespace-nowrap">{$t('containers.system.hawserUpdateRemoteHost')}</p>
													<a
													...
													>
														...
														{$t('containers.system.updateInstructionsGitHub')}
													</a>
												</div>
											{:else}
												<p class="text-sm whitespace-nowrap">{$t('containers.system.hawserRemoteAgent')}</p>
											{/if}
										{/if}
									</Tooltip.Content>
								</Tooltip.Root>
					...
					{:else if column.id === 'image'}
						<div class="flex items-center gap-1.5 {$appSettings.highlightUpdates && containersWithUpdatesSet.has(container.id) ? 'update-border' : ''}">
							{#if containersWithUpdatesSet.has(container.id)}
								<span title={$t('containers.update.available')}>
					...
					{:else if column.id === 'state'}
						...
						<span class="{getStatusClasses(container.state)} inline-flex items-center gap-1">
							...
							{$t(getStatusLabelKey(container.state))}
						</span>
					{:else if column.id === 'health'}
						{#if container.health}
							<div class="flex items-center justify-center" title={$t(getHealthLabelKey(container.health))}>
					...
					{:else if column.id === 'restartCount'}
						{#if container.restartCount > 0}
							<span class="text-xs text-red-500 text-center block" title={$t('containers.tooltips.restarts', { count: container.restartCount })}>{container.restartCount}</span>
						{:else}
					...
					{:else if column.id === 'networkIO'}
						<div class="{isFieldHighlighted(container.id, 'network') ? 'stat-highlight' : ''} text-right whitespace-nowrap">
							{#if containerStats.get(container.id)}
								{@const stats = containerStats.get(container.id)}
								<span class="text-xs font-mono text-muted-foreground" title={$t('containers.tooltips.networkIo', { received: formatBytes(stats.networkRx), sent: formatBytes(stats.networkTx) })}>
					...
					{:else if column.id === 'diskIO'}
						<div class="{isFieldHighlighted(container.id, 'disk') ? 'stat-highlight' : ''} text-right whitespace-nowrap">
							{#if containerStats.get(container.id)}
								{@const stats = containerStats.get(container.id)}
								<span class="text-xs font-mono text-muted-foreground" title={$t('containers.tooltips.diskIo', { read: formatBytes(stats.blockRead), written: formatBytes(stats.blockWrite) })}>
							...
							<Tooltip.Root>
								...
								<Tooltip.Content side="top" class="max-w-none">
									{#each networkEntries as [name, net]}
										<div class="font-mono text-xs">{name}: {net.ipAddress || $t('containers.network.noIp')}</div>
									{/each}
								</Tooltip.Content>
							</Tooltip.Root>
						{:else}
							...
							<div class="flex {compactPorts ? 'flex-nowrap' : 'flex-wrap'} gap-1">
								{#if parsedUrl}
									<a
										...
										title={$t('containers.tooltips.openUrlInNewTab', { url: parsedUrl.url })}
									>
								...
								{#each displayPorts as port}
									...
									{#if url}
										<a
											...
											title={$t('containers.tooltips.openUrlInNewTab', { url })}
										>
								...
								{#each exposedPorts as port}
									<code class="text-xs bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20 px-1 py-0.5 rounded shrink-0" title={$t('containers.tooltips.exposedInternalPort')}>{port.display}</code>
								{/each}
							</div>
						{:else}
							<span class="text-gray-400 dark:text-gray-600 text-xs">-</span>
							...
							<div class="flex items-center justify-center gap-1">
								...
								{#if envHasScanning}
									...
									{#if icon}
										...
										<span class="cursor-default" title={$t(`vulnerabilityCriteria.titles.${criteria}`)}>
						...
						<div class="relative flex gap-0.5 justify-end">
							{#if containersWithUpdatesSet.has(container.id) && !container.systemContainer}
								<ConfirmPopover
									...
									action={$t('containers.actions.update')}
									itemType={$t('common.labels.container')}
									itemName={container.name}
									title={$t('containers.actions.updateAvailableClick')}
							...
							{#if hasActiveLogs(container.id)}
								<button
									...
									title={$t('containers.actions.showLogs')}
							...
							{:else}
								<button
									...
									title={$t('containers.actions.openLogs')}
									class="p-0.5 rounded hover:bg-muted transition-colors opacity-70 hover:opacity-100 cursor-pointer"
								>
									...
									<Popover.Content class="w-56 p-0" align="end" sideOffset={5}>
										...
										{#if detectingShellsFor === container.id}
											<div class="p-4 text-center">
												...
												<p class="text-xs text-muted-foreground">{$t('containers.terminal.detectingShells')}</p>
											</div>
										{:else if !anyShellAvailableFor(container.id)}
											<div class="p-4 text-center">
												<AlertCircle class="w-5 h-5 mx-auto mb-2 text-amber-500" />
												<p class="text-xs font-medium text-amber-500">{$t('containers.terminal.noShellAvailable')}</p>
												<p class="text-xs text-muted-foreground mt-1">{$t('containers.terminal.noShellInstalled')}</p>
											</div>
										{:else}
											<div class="p-3 space-y-3">
												<div class="space-y-1.5">
													<Label class="text-xs">{$t('common.labels.shell')}</Label>
													<Select.Root type="single" bind:value={terminalShell}>
														<Select.Trigger class="w-full h-8 text-xs">
															...
															<span>{shellDetectionCache[container.id]?.allShells.find(o => o.path === terminalShell)?.label || $t('common.actions.select')}</span>
														</Select.Trigger>
														<Select.Content>
												...
												<div class="space-y-1.5">
													<Label class="text-xs">{$t('common.labels.user')}</Label>
													<Select.Root type="single" bind:value={terminalUser}>
														<Select.Trigger class="w-full h-8 text-xs">
															<User class="w-3 h-3 mr-1.5 text-muted-foreground" />
															<span>{userOptions.find(o => o.value === terminalUser)?.label || terminalUser || $t('common.actions.select')}</span>
														</Select.Trigger>
														<Select.Content>
																...
																{#each terminalCustomUsers as cu}
																	<div class="flex items-center group">
																		...
																		<button
																			...
																			title={$t('containers.terminal.removeUser')}
																		>
																			<Trash2 class="w-3 h-3" />
																		</button>
																	</div>
															...
															<div class="px-2 py-1">
																<Input
																	class="h-7 text-xs"
																	placeholder={$t('containers.terminal.addUserPlaceholder')}
							...
							<ConfirmPopover
								...
								action={$t('containers.actions.delete')}
								itemType={$t('common.labels.container')}
								itemName={container.name}
