		...
		<div class="flex items-start gap-2">
			...
			<div class="space-y-2 text-xs">
				{#if systemContainer === 'dockhand'}
					<p class="font-medium text-blue-600 dark:text-blue-400">{$t('containers.autoUpdate.notAvailable')}</p>
					<p class="text-muted-foreground">
						{$t('containers.autoUpdate.dockhandCannotSelfUpdate')}
					</p>
				...
				{:else}
					<p class="font-medium text-blue-600 dark:text-blue-400">{$t('containers.autoUpdate.notAvailable')}</p>
					<p class="text-muted-foreground">
						{$t('containers.autoUpdate.hawserUpdateRemoteHost')}
					</p>
					<a
					...
					>
						...
						{$t('containers.autoUpdate.viewGitHubInstructions')}
					</a>
				{/if}
			</div>
		</div>
...
{:else}
	<div class="space-y-3">
		<div class="flex items-center gap-3">
			<Label class="text-xs font-normal">{$t('containers.autoUpdate.enableAutomatic')}</Label>
			<TogglePill
		...
		{#if enabled}
			...
			{#if envHasScanning}
				<div class="space-y-1.5">
					<Label class="text-xs font-medium">{$t('containers.autoUpdate.vulnerabilityCriteria')}</Label>
