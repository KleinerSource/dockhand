...
>
	...
	<div class="text-xs space-y-1">
		{#if ipv4Address}
			<div>
				<span class="text-muted-foreground">{$t('containers.network.ipv4')}: </span>
				<code class="text-foreground">{ipv4Address}</code>
			</div>
		{/if}
		{#if ipv6Address}
			<div>
				<span class="text-muted-foreground">{$t('containers.network.ipv6')}: </span>
				<code class="text-foreground">{ipv6Address}</code>
			</div>
		{/if}
		{#if macAddress}
			<div>
				<span class="text-muted-foreground">{$t('containers.network.mac')}: </span>
