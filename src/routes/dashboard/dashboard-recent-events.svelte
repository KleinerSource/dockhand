<script lang="ts">
	...
	function formatTime(timestamp: string): string {
		...
		if (diffMins < 1) return $t('relativeTime.short.now');
		if (diffMins < 60) return $t('relativeTime.short.minutesAgo', { count: diffMins });

		const diffHours = Math.floor(diffMins / 60);
		if (diffHours < 24) return $t('relativeTime.short.hoursAgo', { count: diffHours });

		const diffDays = Math.floor(diffHours / 24);
		return $t('relativeTime.short.daysAgo', { count: diffDays });
	}
