...
import type { FavoriteGroup } from '../api/preferences/favorite-groups/+server';
	...
	const layoutModeOptions = $derived([
		{ value: 'single', label: $t('logs.layout.single'), icon: Square },
		{ value: 'multi', label: $t('logs.layout.multi'), icon: LayoutList },
		{ value: 'grouped', label: $t('logs.layout.grouped'), icon: Layers }
	]);
	...
	async function fetchLogs(tail?: number | string) {
		...
		try {
			...
			const data = await response.json();
			logs = data.logs || translate('logs.empty.noLogs');
			scrollToBottom(true); // Force scroll on initial load
		} catch (error) {
			console.error('Failed to fetch logs:', error);
			logs = translate('logs.errors.fetchWithMessage', { error: String(error) });
		} finally {
			loading = false;
		}
	}
	...
	function startStreaming() {
		...
		try {
			...
			connectionError = translate('logs.stream.ended');
			});
			...
			connectionError = translate('logs.stream.startFailed');
			isConnected = false;
			loading = false;
		}
	}
	...
	function handleStreamError() {
		...
		if (reconnectAttempts < MAX_RECONNECT_ATTEMPTS) {
			reconnectAttempts++;
			connectionError = translate('logs.stream.reconnecting', { attempt: reconnectAttempts, total: MAX_RECONNECT_ATTEMPTS });
...
{#if $environments.length === 0 || !$currentEnvironment}
	<div class="flex flex-col flex-1 min-h-0 h-full">
		<PageHeader icon={ScrollText} title={$t('logs.title')} class="h-9 mb-3" />
		<NoEnvironment />
	</div>
{:else}
<div class="flex flex-col flex-1 min-h-0 h-full gap-3">
		...
		{#if layoutMode === 'single'}
			<div class="relative flex-1 max-w-md">
				...
				<div class="relative">
					...
					<Input
						type="text"
						placeholder={selectedContainer ? `${selectedContainer.name} (${selectedContainer.image})` : $t('logs.searchContainers')}
					...
					<div class="absolute top-full left-0 right-0 mt-1 border rounded-md bg-popover shadow-lg z-50 max-h-64 overflow-auto">
						{#if filteredContainers().length === 0}
							<div class="px-3 py-2 text-sm text-muted-foreground">
								{containers.length === 0 ? $t('logs.empty.noContainers') : $t('terminal.noMatches')}
							</div>
						{:else}
							{#each filteredContainers() as container}
								...
								>
									...
									{#if isCurrentSelection}
										<span class="ml-auto text-xs text-muted-foreground">{$t('logs.selection.current')}</span>
									{/if}
								</button>
							{/each}
						{/if}
	...
	<div class="flex-1 min-h-0 flex gap-3">
			...
			<div class="w-64 shrink-0 border rounded-lg overflow-hidden flex flex-col bg-background">
				<div class="px-3 py-2 border-b bg-muted/30">
					<div class="relative">
						...
						<Input
							type="text"
							placeholder={$t('logs.filterContainers')}
							bind:value={searchQuery}
				...
				{#if layoutMode === 'grouped'}
					...
					<div class="px-2 py-1.5 border-b bg-muted/20 flex items-center gap-2">
						...
						>
							{$t('logs.selection.selectAllContainers')}
						</button>
								...
								{#each validFavoriteGroups as savedGroup, idx (savedGroup.name || `group-${idx}`)}
									...
									>
										...
										<div class="flex-1 min-w-0">
											...
											<div class="text-2xs text-muted-foreground truncate leading-tight">{$t('images.usage.containers', { count: savedGroup.containers.length })}</div>
										</div>
										<button
											type="button"
											...
											title={$t('logs.groups.deleteGroup')}
										>
								...
								{#each favoriteContainers() as container}
									...
									>
										...
										<button
											...
											title={$t('logs.tooltips.selectForMerge')}
										>
											{#if isMultiSelected}
				...
				{#if layoutMode === 'grouped'}
					...
					<div class="px-2 py-2 border-t bg-muted/30 flex flex-col gap-2">
						<div class="flex items-center justify-between text-xs text-muted-foreground">
							<span>{$t('logs.selection.selected', { count: selectedContainerIds.size })}</span>
							<span>{$t('logs.selection.containerCount', { count: containers.length })}</span>
						</div>
						{#if selectedContainerIds.size > 0}
							...
							{#if showSaveGroupInput}
								<div class="flex items-center gap-1">
									<input
										type="text"
										placeholder={$t('logs.groups.namePlaceholder')}
					...
					<div class="px-2 py-2 border-t bg-muted/30 flex flex-col gap-2">
						<div class="flex items-center justify-between text-xs text-muted-foreground">
							{#if multiModeSelections.size > 0}
								<span>{$t('logs.selection.selected', { count: multiModeSelections.size })}</span>
								<button
									type="button"
						...
						{#if multiModeSelections.size >= 2}
							<Button size="sm" variant="default" onclick={mergeSelectedContainers} class="w-full h-7 gap-1.5 text-xs">
								...
								{$t('logs.merge.mergeSelected', { count: multiModeSelections.size })}
							</Button>
						{/if}
					</div>
				{/if}
		...
		<div class="{terminalOpen ? 'min-h-0 min-w-0' : 'flex-1'} border rounded-lg overflow-hidden flex flex-col transition-colors {darkMode ? 'bg-zinc-950 border-zinc-800' : 'bg-gray-50 border-gray-300'}" style="{terminalOpen ? (terminalLayout === 'below' ? `height: ${terminalSplitRatio * 100}%` : `width: ${terminalSplitRatio * 100}%`) : ''}">
			{#if layoutMode === 'grouped'}
				{#if selectedContainerIds.size === 0}
					<div class="flex items-center justify-center h-full text-muted-foreground">
						{$t('logs.empty.selectContainersForMergedLogs')}
					</div>
				{:else}
					...
					<div class="flex items-center flex-wrap gap-y-1 px-3 py-1.5 border-b shrink-0 transition-colors {darkMode ? 'border-zinc-800 bg-zinc-900/50' : 'border-gray-300 bg-gray-100'}">
						<div class="flex items-center gap-2 shrink-0">
							{#if streamingEnabled}
								{#if isConnected}
									<div class="flex items-center gap-1.5" title={$t('logs.stream.connectedTooltip')}>
										<Wifi class="w-3.5 h-3.5 text-green-500" />
										<span class="text-xs text-green-500 font-medium">{$t('logs.stream.live')}</span>
									</div>
								{:else if loading}
									<div class="flex items-center gap-1.5" title={$t('common.states.connecting')}>
										<RefreshCw class="w-3.5 h-3.5 animate-spin {darkMode ? 'text-amber-500' : 'text-amber-600'}" />
										<span class="text-xs {darkMode ? 'text-amber-500' : 'text-amber-600'}">{$t('common.states.connecting')}...</span>
									</div>
								{:else if connectionError}
									<button onclick={retryConnection} class="flex items-center gap-1.5 hover:opacity-80" title={connectionError}>
										...
										<span class="text-xs {darkMode ? 'text-zinc-500' : 'text-gray-400'}">{$t('terminal.disconnected')}</span>
									</button>
								{/if}
							{:else}
								<div class="flex items-center gap-1.5" title={$t('logs.stream.pausedTooltip')}>
									<Pause class="w-3.5 h-3.5 {darkMode ? 'text-zinc-500' : 'text-gray-400'}" />
									<span class="text-xs {darkMode ? 'text-zinc-500' : 'text-gray-400'}">{$t('logs.stream.paused')}</span>
								</div>
							{/if}
						...
						<div class="flex items-center gap-2 flex-wrap ml-auto">
							<button
								...
								title={streamingEnabled ? $t('logs.tooltips.pauseStreaming') : $t('logs.tooltips.resumeStreaming')}
							>
								{#if streamingEnabled}
									<Pause class="w-3 h-3" />
									<span>{$t('logs.stream.pause')}</span>
								{:else}
									<Play class="w-3 h-3" />
									<span>{$t('logs.stream.stream')}</span>
								{/if}
							</button>
							<button
								...
								title={$t('logs.tooltips.toggleAutoScroll')}
							>
								<ArrowDownToLine class="w-3 h-3" />
								<span>{$t('logs.controls.autoScroll')}</span>
							</button>
							<!-- Tail lines selector -->
							<Select.Root type="single" value={tailCount} onValueChange={(v) => { tailCount = v; saveState(); reloadAllLogs(); }}>
								<Select.Trigger class="!h-7 w-[52px] text-xs px-2 [&_svg]:size-3 {darkMode ? 'bg-zinc-800 border-zinc-700 text-zinc-300' : 'bg-white border-gray-300 text-gray-700'}" title={$t('logs.tooltips.tailLines')}>
									<span>{tailCount === 'all' ? $t('logs.tail.all') : tailOptions.find(o => o.value === tailCount)?.label ?? tailCount}</span>
								</Select.Trigger>
								<Select.Content>
							...
							<button
								...
								title={showTimestamps ? $t('logs.tooltips.hideTimestamps') : $t('logs.tooltips.showTimestamps')}
							>
							...
							<button
								...
								title={showContainerName ? $t('logs.tooltips.hideContainerName') : $t('logs.tooltips.showContainerName')}
							>
							...
							<button
								...
								title={showLineNumbers ? $t('logs.tooltips.hideLineNumbers') : $t('logs.tooltips.showLineNumbers')}
							>
							...
							<button onclick={toggleTheme} class="p-1 rounded transition-colors {darkMode ? 'hover:bg-zinc-800' : 'hover:bg-gray-200'}" title={darkMode ? $t('themeSelector.lightTheme') : $t('themeSelector.darkTheme')}>
								{#if darkMode}
							...
							{#if logSearchActive}
								<div class="flex items-center gap-1.5 rounded px-2 py-1 {darkMode ? 'bg-zinc-800' : 'bg-gray-200'}">
									...
									<button
										...
										title={logSearchFilterMode ? $t('logs.tooltips.showAllLines') : $t('logs.tooltips.hideNonMatchingLines')}
									>
									...
									{/if}
									<button onclick={() => navigateMatch('prev')} class="p-0.5 rounded transition-colors {darkMode ? 'hover:bg-zinc-700' : 'hover:bg-gray-300'}" title={$t('logs.search.previous')}>
										<ChevronUp class="w-3 h-3 {darkMode ? 'text-zinc-400' : 'text-gray-500'}" />
									</button>
									<button onclick={() => navigateMatch('next')} class="p-0.5 rounded transition-colors {darkMode ? 'hover:bg-zinc-700' : 'hover:bg-gray-300'}" title={$t('logs.search.next')}>
										<ChevronDown class="w-3 h-3 {darkMode ? 'text-zinc-400' : 'text-gray-500'}" />
									</button>
									<button onclick={closeLogSearch} class="p-0.5 rounded transition-colors {darkMode ? 'hover:bg-zinc-700' : 'hover:bg-gray-300'}" title={$t('logs.search.close')}>
										<X class="w-3 h-3 {darkMode ? 'text-zinc-400' : 'text-gray-500'}" />
									</button>
								</div>
							{:else}
								<button onclick={toggleLogSearch} class="p-1 rounded transition-colors {darkMode ? 'hover:bg-zinc-800' : 'hover:bg-gray-200'}" title={$t('logs.tooltips.searchLogs')}>
									<Search class="w-3 h-3 {darkMode ? 'text-zinc-500 hover:text-zinc-300' : 'text-gray-500 hover:text-gray-700'}" />
								</button>
							{/if}
							<button onclick={copyLogs} class="p-1 rounded transition-colors {darkMode ? 'hover:bg-zinc-800' : 'hover:bg-gray-200'}" title={$t('logs.tooltips.copyLogs')}>
								<Copy class="w-3 h-3 {darkMode ? 'text-zinc-500 hover:text-zinc-300' : 'text-gray-500 hover:text-gray-700'}" />
							</button>
							<button onclick={downloadLogs} class="p-1 rounded transition-colors {darkMode ? 'hover:bg-zinc-800' : 'hover:bg-gray-200'}" title={$t('logs.tooltips.downloadLogs')}>
								<Download class="w-3 h-3 {darkMode ? 'text-zinc-500 hover:text-zinc-300' : 'text-gray-500 hover:text-gray-700'}" />
							</button>
							<button onclick={clearLogs} class="p-1 rounded transition-colors {darkMode ? 'hover:bg-zinc-800' : 'hover:bg-gray-200'}" title={$t('logs.tooltips.clearLogs')}>
			...
			{:else if !selectedContainer}
				<div class="flex items-center justify-center h-full text-muted-foreground">
					{layoutMode === 'multi' ? $t('logs.empty.selectContainerFromList') : $t('logs.empty.selectContainer')}
				</div>
			{:else}
			...
			<div class="flex items-center flex-wrap gap-y-1 px-3 py-1.5 border-b shrink-0 transition-colors {darkMode ? 'border-zinc-800 bg-zinc-900/50' : 'border-gray-300 bg-gray-100'}">
				<div class="flex items-center gap-2 shrink-0">
					...
					{#if streamingEnabled}
						{#if isConnected}
							<div class="flex items-center gap-1.5 transition-opacity duration-300" title={$t('logs.stream.connectedTooltip')}>
								<Wifi class="w-3.5 h-3.5 text-green-500" />
								<span class="text-xs text-green-500 font-medium">{$t('logs.stream.live')}</span>
							</div>
						{:else if loading}
							<div class="flex items-center gap-1.5 transition-opacity duration-300" title={$t('common.states.connecting')}>
								<RefreshCw class="w-3.5 h-3.5 animate-spin {darkMode ? 'text-amber-500' : 'text-amber-600'}" />
								<span class="text-xs {darkMode ? 'text-amber-500' : 'text-amber-600'}">{$t('common.states.connecting')}...</span>
							</div>
						{:else if connectionError}
						...
						{:else}
							<div class="flex items-center gap-1.5 transition-opacity duration-300" title={$t('terminal.disconnected')}>
								<WifiOff class="w-3.5 h-3.5 {darkMode ? 'text-zinc-500' : 'text-gray-400'}" />
								<span class="text-xs {darkMode ? 'text-zinc-500' : 'text-gray-400'}">{$t('common.states.offline')}</span>
							</div>
						{/if}
					{:else}
						<div class="flex items-center gap-1.5 transition-opacity duration-300" title={$t('logs.stream.pausedTooltip')}>
							<Pause class="w-3.5 h-3.5 {darkMode ? 'text-zinc-500' : 'text-gray-400'}" />
							<span class="text-xs {darkMode ? 'text-zinc-500' : 'text-gray-400'}">{$t('logs.stream.paused')}</span>
						</div>
					{/if}
					...
					{#if selectedContainer}
						<div class="flex items-center gap-1.5 ml-2">
							...
							<button
								...
								title={$t('logs.tooltips.terminalBelow')}
							>
							...
							<button
								...
								title={$t('logs.tooltips.terminalOnSide')}
							>
								<span class="inline-flex items-center gap-px">
				...
				<div class="flex items-center gap-2 flex-wrap ml-auto">
					...
					<button
						...
						title={darkMode ? $t('themeSelector.lightTheme') : $t('themeSelector.darkTheme')}
					>
						{#if darkMode}
							<Sun class="w-3 h-3 {darkMode ? 'text-zinc-500 hover:text-zinc-300' : 'text-gray-500 hover:text-gray-700'}" />
						{:else}
					...
					{:else}
						<button
							...
							class="p-1 rounded transition-colors {darkMode ? 'hover:bg-zinc-800' : 'hover:bg-gray-200'}"
							title={$t('logs.tooltips.searchLogs')}
						>
							<Search class="w-3 h-3 {darkMode ? 'text-zinc-500 hover:text-zinc-300' : 'text-gray-500 hover:text-gray-700'}" />
						</button>
					{/if}
					<button
						onclick={copyLogs}
						class="p-1 rounded transition-colors {darkMode ? 'hover:bg-zinc-800' : 'hover:bg-gray-200'}"
						title={$t('logs.tooltips.copyLogs')}
					>
						<Copy class="w-3 h-3 {darkMode ? 'text-zinc-500 hover:text-zinc-300' : 'text-gray-500 hover:text-gray-700'}" />
					</button>
					<button
						...
						class="p-1 rounded transition-colors {darkMode ? 'hover:bg-zinc-800' : 'hover:bg-gray-200'}"
						title={$t('logs.tooltips.downloadLogs')}
					>
						<Download class="w-3 h-3 {darkMode ? 'text-zinc-500 hover:text-zinc-300' : 'text-gray-500 hover:text-gray-700'}" />
					</button>
					<button
						onclick={clearLogs}
						class="p-1 rounded transition-colors {darkMode ? 'hover:bg-zinc-800' : 'hover:bg-gray-200'}"
						title={$t('logs.tooltips.clearLogs')}
					>
						<Eraser class="w-3 h-3 {darkMode ? 'text-zinc-500 hover:text-zinc-300' : 'text-gray-500 hover:text-gray-700'}" />
					</button>
					<button
						onclick={() => fetchLogs()}
						class="p-1 rounded transition-colors {darkMode ? 'hover:bg-zinc-800' : 'hover:bg-gray-200'}"
						title={$t('logs.tooltips.refreshLogs')}
					>
