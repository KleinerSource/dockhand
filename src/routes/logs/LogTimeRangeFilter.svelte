...
<Popover.Root bind:open onOpenChange={(v) => { if (v) picking = sinceDate && !untilDate ? 'to' : 'from'; }}>
	<Popover.Trigger
		...
		title={hasFilter ? $t('logs.timeRange.activeTooltip', { range: formatLabel() }) : $t('logs.timeRange.tooltip')}
	>
	...
	<Popover.Content class="w-auto p-0 z-[200]" align="start">
		<div class="flex flex-col">
			...
			<div class="flex border-b border-border">
				...
				>
					{$t('filters.from')}{#if sinceDate}<span class="ml-1 text-muted-foreground">({sinceDate.slice(5)})</span>{/if}
				</button>
				<button
					type="button"
					onclick={() => picking = 'to'}
				...
				>
					{$t('filters.to')}{#if untilDate}<span class="ml-1 text-muted-foreground">({untilDate.slice(5)})</span>{/if}
				</button>
			</div>
			...
			{#if picking === 'from'}
				...
				<div class="px-3 pb-2">
					<div class="flex items-center gap-2">
						<span class="text-xs text-muted-foreground">{$t('logs.timeRange.time')}:</span>
			...
			<div class="flex items-center gap-2 px-3 pb-2">
				<button onclick={apply} class="px-2 py-1 rounded text-xs bg-primary text-primary-foreground hover:bg-primary/90" disabled={!sinceDate && !untilDate}>{$t('common.actions.apply')}</button>
				<button onclick={clear} class="px-2 py-1 rounded text-xs bg-muted text-muted-foreground hover:bg-muted/80">{$t('common.actions.clear')}</button>
			</div>
		</div>
	</Popover.Content>
</Popover.Root>
