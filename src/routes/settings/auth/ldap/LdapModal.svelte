<script lang="ts">
	...
	async function save() {
		...
		if (!formName.trim()) {
			formErrors.name = translate('settings.auth.ldap.modal.validation.nameRequired');
			hasErrors = true;
		}
		if (!formServerUrl.trim()) {
			formErrors.serverUrl = translate('settings.auth.ldap.modal.validation.serverUrlRequired');
			hasErrors = true;
		}
		if (!formBaseDn.trim()) {
			formErrors.baseDn = translate('settings.auth.ldap.modal.validation.baseDnRequired');
			hasErrors = true;
		}
		...
		try {
			...
			if (response.ok) {
				...
				formError = data.error || translate(isEditing ? 'settings.auth.ldap.modal.errors.updateFailed' : 'settings.auth.ldap.modal.errors.createFailed');
			}
		} catch {
			formError = translate(isEditing ? 'settings.auth.ldap.modal.errors.updateFailed' : 'settings.auth.ldap.modal.errors.createFailed');
		} finally {
			formSaving = false;
		}
	}
...
<Dialog.Root bind:open onOpenChange={(o) => { if (o) { formError = ''; formErrors = {}; formModalTab = 'connection'; focusFirstInput(); } }}>
	<Dialog.Content class="max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
		<Dialog.Header class="flex-shrink-0">
			<Dialog.Title class="flex items-center gap-2">
				{#if isEditing}
					...
					{$t('settings.auth.ldap.modal.titleEdit')}
				{:else}
					<Network class="w-5 h-5" />
					{$t('settings.auth.ldap.modal.titleAdd')}
				{/if}
			</Dialog.Title>
		</Dialog.Header>
		<div class="flex-1 overflow-y-auto py-2">
			...
			<Tabs.Root bind:value={formModalTab}>
				<Tabs.List class="grid w-full grid-cols-2 mb-4">
					<Tabs.Trigger value="connection">{$t('settings.auth.ldap.modal.tabs.connection')}</Tabs.Trigger>
					<Tabs.Trigger value="groups">{$t('settings.auth.ldap.modal.tabs.groups')}</Tabs.Trigger>
				</Tabs.List>

				<Tabs.Content value="connection" class="space-y-4">
					<!-- Basic Settings -->
					<div class="space-y-4">
						<h4 class="text-sm font-medium text-muted-foreground">{$t('settings.auth.ldap.modal.basicSettings')}</h4>
						<div class="grid grid-cols-2 gap-4">
							<div class="space-y-2">
								<Label>{$t('settings.auth.ldap.modal.name')} <span class="text-destructive">*</span></Label>
								<Input
									bind:value={formName}
									placeholder={$t('settings.auth.ldap.modal.namePlaceholder')}
							...
							<div class="space-y-2">
								<Label>{$t('settings.auth.ldap.modal.serverUrl')} <span class="text-destructive">*</span></Label>
								<Input
						...
						<div class="flex items-center gap-2">
							...
							<Label class="text-sm font-normal cursor-pointer" onclick={() => formEnabled = !formEnabled}>
								{$t('settings.auth.ldap.modal.enableConfiguration')}
							</Label>
						</div>
					</div>
					...
					<!-- Bind Credentials -->
					<div class="space-y-4">
						<h4 class="text-sm font-medium text-muted-foreground">{$t('settings.auth.ldap.modal.bindCredentials')}</h4>
						<p class="text-xs text-muted-foreground">{$t('settings.auth.ldap.modal.bindCredentialsDescription')}</p>
						<div class="grid grid-cols-2 gap-4">
							<div class="space-y-2">
								<Label>{$t('settings.auth.ldap.modal.bindDn')}</Label>
								<Input
							...
							<div class="space-y-2">
								<Label>{$t('settings.auth.ldap.modal.bindPassword')}</Label>
								<Input
									type="password"
									bind:value={formBindPassword}
									placeholder={isEditing ? $t('settings.auth.ldap.modal.keepExistingPassword') : $t('settings.auth.ldap.modal.enterPassword')}
								/>
							</div>
						</div>
					</div>
					...
					<!-- Search Settings -->
					<div class="space-y-4">
						<h4 class="text-sm font-medium text-muted-foreground">{$t('settings.auth.ldap.modal.userSearchSettings')}</h4>
						<div class="space-y-2">
							<Label>{$t('settings.auth.ldap.modal.baseDn')} <span class="text-destructive">*</span></Label>
							<Input
							...
							{:else}
								<p class="text-xs text-muted-foreground">{$t('settings.auth.ldap.modal.baseDnHelp')}</p>
							{/if}
						</div>
						<div class="space-y-2">
							<Label>{$t('settings.auth.ldap.modal.userFilter')}</Label>
							<Input
								bind:value={formUserFilter}
								placeholder={`(uid={{username}})`}
							/>
							<p class="text-xs text-muted-foreground">
								{$t('settings.auth.ldap.modal.userFilterHelpBefore')} <code class="text-xs bg-muted px-1 rounded">{`{{username}}`}</code> {$t('settings.auth.ldap.modal.userFilterHelpAfter')}<br />
					...
					<!-- Attribute Mapping -->
					<div class="space-y-4">
						<h4 class="text-sm font-medium text-muted-foreground">{$t('settings.auth.ldap.modal.attributeMapping')}</h4>
						<div class="grid grid-cols-3 gap-4">
							<div class="space-y-2">
								<Label>{$t('settings.auth.ldap.modal.usernameAttribute')}</Label>
								<Input
							...
							<div class="space-y-2">
								<Label>{$t('settings.auth.ldap.modal.emailAttribute')}</Label>
								<Input
							...
							<div class="space-y-2">
								<Label>{$t('settings.auth.ldap.modal.displayNameAttribute')}</Label>
								<Input
					...
					<!-- TLS Settings -->
					<div class="space-y-4">
						<h4 class="text-sm font-medium text-muted-foreground">{$t('settings.auth.ldap.modal.tlsSettings')}</h4>
						<div class="flex items-center gap-2">
							<Checkbox
							...
							<Label class="text-sm font-normal cursor-pointer" onclick={() => formTlsEnabled = !formTlsEnabled}>
								{$t('settings.auth.ldap.modal.enableTls')}
							</Label>
						</div>
						{#if formTlsEnabled}
							<div class="space-y-2">
								<Label>{$t('settings.auth.ldap.modal.caCertificateOptional')}</Label>
								<textarea
				...
				<Tabs.Content value="groups" class="space-y-4">
					<!-- Group Settings -->
					<div class="space-y-4">
						<h4 class="text-sm font-medium text-muted-foreground">{$t('settings.auth.ldap.modal.groupSettings')}</h4>
						<p class="text-xs text-muted-foreground">{$t('settings.auth.ldap.modal.groupSettingsDescription')}</p>
						<div class="grid grid-cols-2 gap-4">
							<div class="space-y-2">
								<Label>{$t('settings.auth.ldap.modal.groupBaseDn')}</Label>
								<Input
									bind:value={formGroupBaseDn}
									placeholder="ou=groups,dc=example,dc=com"
								/>
								<p class="text-xs text-muted-foreground">{$t('settings.auth.ldap.modal.groupBaseDnHelp')}</p>
							</div>
							<div class="space-y-2">
								<Label>{$t('settings.auth.ldap.modal.adminGroup')}</Label>
								<Input
									bind:value={formAdminGroup}
									placeholder="cn=admins,ou=groups,dc=example,dc=com"
								/>
								<p class="text-xs text-muted-foreground">{$t('settings.auth.ldap.modal.adminGroupHelp')}</p>
							</div>
						</div>
						<div class="space-y-2">
							<Label>{$t('settings.auth.ldap.modal.memberFilter')}</Label>
							<Input
								bind:value={formGroupFilter}
								placeholder={'(&(objectClass=groupOfNames)(member={{user_dn}}))'}
							/>
							<p class="text-xs text-muted-foreground">
								{$t('settings.auth.ldap.modal.memberFilterHelpBefore')} <code class="text-xs bg-muted px-1 rounded">{'{{user_dn}}'}</code> {$t('settings.auth.ldap.modal.memberFilterHelpAfter')}
							</p>
						</div>
					</div>
					...
					{#if isEnterprise}
						<div class="space-y-4">
							<div class="flex items-center gap-2">
								<h4 class="text-sm font-medium text-muted-foreground">{$t('settings.auth.ldap.modal.groupRoleMappings')}</h4>
								<Star class="w-3.5 h-3.5 text-amber-500" />
							</div>
							<p class="text-xs text-muted-foreground">{$t('settings.auth.ldap.modal.groupRoleMappingsDescription')}</p>
											...
											>
												<Select.Trigger class="w-40">
													{#if mapping.roleId}
														...
														{role?.name || $t('settings.auth.ldap.modal.selectRole')}
													{:else}
														{$t('settings.auth.ldap.modal.selectRole')}
													{/if}
												</Select.Trigger>
												<Select.Content>
							...
							<Button variant="outline" size="sm" onclick={addRoleMapping}>
								<Plus class="w-4 h-4" />
								{$t('settings.auth.ldap.modal.addMapping')}
							</Button>
						</div>
					{/if}
				</Tabs.Content>
		...
		<Dialog.Footer class="flex-shrink-0 border-t pt-4">
			<Button variant="outline" onclick={handleClose}>{$t('settings.auth.ldap.modal.cancel')}</Button>
			<Button onclick={save} disabled={formSaving}>
				...
				{:else}
					<Plus class="w-4 h-4" />
				{/if}
				{isEditing ? $t('settings.auth.ldap.modal.save') : $t('settings.auth.ldap.modal.addConfiguration')}
			</Button>
		</Dialog.Footer>
	</Dialog.Content>
</Dialog.Root>
