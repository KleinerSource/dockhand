...
`;
	...
	const logMaxLinesOptions = $derived([
		{ value: '500', label: $t('logs.tail.lines', { count: '500' }) },
		{ value: '1000', label: $t('logs.tail.lines', { count: '1,000' }) },
		{ value: '2000', label: $t('logs.tail.lines', { count: '2,000' }) }
	]);
	...
	function handleLogMaxLinesChange(value: string | undefined) {
		...
		toast.success($t('settings.general.logMaxLinesUpdated'));
	}
			...
			<Card.Root>
				...
				<Card.Content>
					<div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
						<div class="space-y-4">
							<div class="space-y-2">
								<Label for="log-max-lines">{$t('settings.general.logMaxLines')}</Label>
								<Select.Root
									type="single"
								...
								>
									<Select.Trigger id="log-max-lines" class="w-48">
										{$t('logs.tail.lines', { count: logMaxLines.toLocaleString() })}
									</Select.Trigger>
									<Select.Content>
								...
								<p class="text-xs text-muted-foreground">{$t('settings.general.logMaxLinesDescription')}</p>
							</div>
							<div class="space-y-1">
								<div class="flex items-center gap-3">
